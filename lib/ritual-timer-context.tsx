    ...
    <>
      <AnimatePresence>
        {isSaving && (
            <motion.div 
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="fixed inset-0 z-[200] bg-[#F6F2EC]/90 backdrop-blur-md flex flex-col items-center justify-center p-6 text-center"
            >
                ...
                <h2 className="text-2xl font-bold text-[#556B5A] mb-2">Saving Ritual</h2>
                <p className="text-[#556B5A]/60 max-w-[240px]">We&apos;re recording your progress. This will only take a moment.</p>
            </motion.div>
        )}
      </AnimatePresence>
      ...
      <Drawer open={isPlayerOpen} onOpenChange={(open) => !open && closePlayer()} repositionInputs={true}>
        <DrawerContent className="bg-[#F6F2EC]/60 backdrop-blur-3xl border-none font-[Outfit] h-[100dvh] max-h-[100dvh] rounded-none z-[100] overflow-hidden flex flex-col">
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[120%] bg-[#556B5A]/5 rounded-full blur-3xl pointer-events-none" />

          <div className="flex flex-col h-full p-6 pb-12 overflow-hidden relative z-10">
            <button onClick={closePlayer} className="absolute top-6 right-6 w-12 h-12 rounded-full bg-white/40 backdrop-blur-xl flex items-center justify-center text-[#556B5A] shadow-sm border border-white/50 active:scale-90 z-20">
            ...
            <AnimatePresence mode="wait">
              {ritual.step === "active" && currentHabit && (
                ...
                >
                  <div className="text-center w-full">
                    <div className="text-[10px] font-black text-[#556B5A]/30 uppercase tracking-[0.2em] mb-4">
                      Habit {ritual.currentIndex + 1} of {ritual.habits.length}
                    </div>
                    <h2 className="text-3xl font-bold text-[#556B5A] px-6 leading-tight">{currentHabit.title}</h2>
                    <p className="text-[#556B5A]/40 text-sm mt-2 font-medium">{currentHabit.description}</p>
                  </div>
                  ...
                  <div className="relative w-64 h-64 flex items-center justify-center">
                    ...
                    <div className="text-6xl font-black text-[#556B5A] tracking-tighter">
                  ...
                  <div className="w-full space-y-8 px-6">
                    <div className="flex items-center justify-center gap-4">
                      <button onClick={reduceTimer} className="w-12 h-12 rounded-full bg-white/40 backdrop-blur-md flex items-center justify-center text-[#556B5A] shadow-sm border border-white/50 active:scale-90">
                        <Minus className="w-5 h-5" />
                      </button>
                      <button onClick={togglePause} className="w-20 h-20 rounded-full bg-white/60 backdrop-blur-xl shadow-xl border border-white/50 flex items-center justify-center text-[#556B5A] active:scale-95">
                        {ritual.isPaused ? <Play className="w-8 h-8 fill-[#556B5A]" /> : <Pause className="w-8 h-8 fill-[#556B5A]" />}
                      </button>
                      <button onClick={extendTimer} className="w-12 h-12 rounded-full bg-white/40 backdrop-blur-md flex items-center justify-center text-[#556B5A] shadow-sm border border-white/50 active:scale-90">
                    ...
                    <div className="flex flex-col gap-3">
                      <div className="flex gap-3">
                        <button onClick={skipHabit} className="flex-1 h-14 rounded-2xl bg-white/30 backdrop-blur-md text-[#556B5A] font-bold border border-white/30 active:scale-[0.98] transition-all">
                          Skip
                        </button>
                        <button onClick={nextHabit} className="flex-1 h-14 rounded-2xl bg-[#556B5A] text-white font-bold shadow-lg active:scale-[0.98] transition-all">
                          Next
                        </button>
                      </div>
                      <button onClick={cancelRitual} className="w-full h-12 rounded-2xl text-[#556B5A]/40 text-xs font-black uppercase tracking-widest hover:text-red-400 transition-colors">
                        Cancel Ritual
                      </button>
                    </div>
                  </div>
              ...
              {ritual.step === "summary" && (
                ...
                >
                  <div className="w-20 h-20 bg-white/40 backdrop-blur-xl border border-white/50 rounded-full flex items-center justify-center mb-6 text-[#7C9A86] shadow-sm">
                    <Check className="w-10 h-10" strokeWidth={3} />
                  </div>
                  <h2 className="text-3xl font-black text-[#556B5A] uppercase tracking-tighter mb-8">Ritual Complete</h2>
                  ...
                  <div className="w-full space-y-3 mb-12 overflow-y-auto no-scrollbar max-h-[40vh]">
                    {ritual.habits.map((habit) => (
                      <div key={habit.id} className="flex items-center justify-between p-5 bg-white/40 backdrop-blur-md border border-white/50 rounded-3xl shadow-sm">
                        <span className="font-bold text-[#556B5A] text-sm">{habit.title}</span>
                        {ritual.skippedIds.includes(habit.id) ? (
                          ...
                          <div className="w-8 h-8 bg-[#7C9A86]/20 rounded-full flex items-center justify-center">
                            <Check className="w-5 h-5 text-[#7C9A86]" strokeWidth={3} />
                          </div>
                        )}
                      </div>
                    ))}
                  ...
                  <button onClick={goToReflection} className="w-full h-16 rounded-full bg-[#556B5A] text-white font-bold text-lg shadow-xl shadow-[#556B5A]/20 active:scale-95">
                    Continue
                  </button>
                </motion.div>
              )}
              ...
              {ritual.step === "reflection" && (
                ...
                >
                  ...
                  <h2 className="text-2xl font-black text-[#556B5A] uppercase tracking-tighter mb-2 leading-none">How do you feel?</h2>
                  <p className="text-xs font-medium text-[#556B5A]/40 mb-6">Capture your energy and thoughts.</p>

                  <div className="w-full space-y-6 text-left">
                      {/* Reflection Word */}
                      <div>
                          <label className="block text-[10px] font-black text-[#556B5A]/40 uppercase tracking-widest mb-2 ml-1">Energy (One Word)</label>
                          <input
                            type="text"
                            value={ritual.reflection}
                            onChange={(event) => setReflection(event.target.value)}
                            placeholder="Grateful, Calm, Ready..."
                            className="w-full p-4 text-xl bg-white/40 backdrop-blur-md border border-white/50 rounded-[20px] text-[#556B5A] font-bold placeholder:text-[#556B5A]/20 outline-none focus:ring-1 focus:ring-[#556B5A]/20 transition-all shadow-sm"
                          />
                      </div>

                      {/* Comment/Notes */}
                      <div>
                          <label className="block text-[10px] font-black text-[#556B5A]/40 uppercase tracking-widest mb-2 ml-1">Optional Notes</label>
                          <textarea 
                              value={ritual.notes || ""}
                              onChange={(e) => setNotes(e.target.value)}
                              placeholder="How did the ritual go? Any deeper reflections?"
                              rows={3}
                              className="w-full bg-white/40 backdrop-blur-md border border-white/50 rounded-[20px] p-4 text-[#556B5A] text-sm outline-none resize-none focus:ring-1 focus:ring-[#556B5A]/20 transition-all shadow-sm font-medium placeholder:text-[#556B5A]/30"
                          />
                      </div>
                          ...
                          {ritual.imageUrl ? (
                              ...
                              >
                                  {isUploading ? (
                                      ...
                                      <>
                                          <div className="w-10 h-10 rounded-full bg-white/60 flex items-center justify-center text-[#556B5A]/40 border border-white/50 shadow-sm">
                                              <Plus className="w-5 h-5" />
                                          </div>
                  ...
                  <button
                    ...
                    className="w-full h-16 rounded-full bg-[#556B5A] text-white font-bold text-lg shadow-xl shadow-[#556B5A]/20 active:scale-95 disabled:opacity-50 flex items-center justify-center mt-8 shrink-0"
                  >
                ...
                <div className="relative flex items-center gap-3 px-4 py-3">
                  ...
                  <div className="min-w-0 flex-1 text-left">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="rounded-full border border-[#556B5A]/8 bg-[#F2E6DE] px-2 py-0.5 text-[10px] font-black uppercase tracking-widest text-[#556B5A]/65">
                        {ritual.currentIndex + 1}/{ritual.habits.length}
                      </span>
                    </div>
                    <p className="truncate text-xs font-bold text-[#556B5A]/55">{currentHabit.title}</p>
                  </div>
                  ...
                  <button
                    ...
                    className="relative z-10 flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-[#556B5A] text-white shadow-lg shadow-[#556B5A]/20 active:scale-95"
