...
function hasDrawerTitle(children: React.ReactNode): boolean {
  return React.Children.toArray(children).some((child) => {
    if (!React.isValidElement(child)) return false

    const props = child.props as { ["data-slot"]?: string; children?: React.ReactNode }

    if (props["data-slot"] === "drawer-title") return true

    if (props.children) {
      return hasDrawerTitle(props.children)
    }
...
function DrawerOverlay({
  ...
  return (
    <DrawerPrimitive.Overlay
      ...
      className={cn(
        "data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 fixed inset-0 z-[100] bg-black/10",
        className
      )}
      {...props}
    />
...
function DrawerContent({
  ...
  const includesTitle = hasDrawerTitle(children)
  ...
  return (
    <DrawerPortal data-slot="drawer-portal">
      ...
      >
        ...
        {!includesTitle ? <DrawerPrimitive.Title className="sr-only">Drawer</DrawerPrimitive.Title> : null}
        {children}
      </DrawerPrimitive.Content>
    </DrawerPortal>
  )
