    ...
    Credentials({
      ...
      authorize: async (credentials) => {
        try {
            ...
            if (isDev) console.log("[AUTH] Authorizing with credentials:", { ...credentials, password: "REDACTED" });
            ...
            if (tokenVal && passwordVal) {
                if (isDev) console.log("[AUTH] Token + Password login attempt:", tokenVal);
                ...
                if (!product || !product.active || !product.user || !product.user.password) {
                    if (isDev) console.log("[AUTH] Token + Password failed: Invalid product or user");
                    return null;
                }

                const passwordsMatch = await bcrypt.compare(passwordVal, product.user.password);
                if (passwordsMatch) {
                    if (isDev) console.log("[AUTH] Token + Password success for:", product.user.email);
                    return {
                        id: product.user.id,
                ...
                if (isDev) console.log("[AUTH] Token + Password failed: Incorrect password");
                return null;
            }

            // 2. Token-only Login (Magic Link / NFC Trusted)
            if (tokenVal && !passwordVal) {
              if (isDev) console.log("[AUTH] Token login attempt:", tokenVal);
              const product = await db.product.findUnique({
                where: { token: tokenVal },
                include: { user: true }
              });
              ...
              if (product && product.active && product.user) {
                if (isDev) console.log("[AUTH] Token login success for user:", product.user.email);
                return {
                    id: product.user.id,
                    email: product.user.email,
                    name: product.user.name,
              ...
              if (isDev) console.log("[AUTH] Token login failed");
              return null;
            }
            ...
            if (emailVal && passwordVal) {
                ...
                if (!parsedCredentials.success) {
                    if (isDev) console.log("[AUTH] Password login failed: Invalid schema", parsedCredentials.error);
                    return null;
                }

                const { email, password } = parsedCredentials.data;
                if (isDev) console.log("[AUTH] Password login attempt for:", email);
                ...
                if (user && user.password) {
                    ...
                    if (passwordsMatch) {
                        if (user.setupRequired) {
                            if (isDev) console.log("[AUTH] Password login failed: Setup required");
                            return null;
                        }

                        if (isDev) console.log("[AUTH] Password login success for user:", user.email);
                        return {
                            id: user.id,
                            email: user.email,
                            name: user.name,
                ...
                if (isDev) console.log("[AUTH] Password login failed");
            }
